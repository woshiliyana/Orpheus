...
本产品当前工作名为 `Orpheus`。它在长期愿景上仍是面向全球长视频创作者的配音工作流产品，但 V1 不再追求“多语言、多赛道、完整平台”的一次性展开，而是收敛为：

1. 英语完整产品
2. 面向 faceless YouTube creators 的 `story-first documentary` 主赛道
3. 首发入口聚焦 `true crime`、`dark mystery`、`dark history`

产品当前有两个核心价值支柱：

1. 长文本工作流：从脚本到可交付音频 / 字幕资产
2. 旗舰音色资产：少量但可长期使用、人格清晰且版本稳定的核心音色
...
2. 把免登录真实试用做成首页和工具页主漏斗，但不做 unlimited free
3. 免费层提供大量 AI 启动库音色选择感，付费层提供旗舰音色、一致性和完整工作流
...
V1 首发用户应优先收敛为英语 faceless YouTube 长视频创作者，重点服务 `story-first documentary` 集群，典型入口包括：
...
1. 商业分析 / rise-and-fall 叙事
2. 心理学 / dark psychology
3. 纪录片式知识解释
...
3. 用户希望在免费层就感受到“可选音色很多”，而不是只看到 2-3 个样本音色。
4. 用户真正愿意付费的原因不是“音色数量更多”，而是音色更稳、更像频道人格、工作流更完整。
5. 用户希望降低剪辑、字幕和重复生成的摩擦。
6. 用户对价格敏感，尤其是在长脚本场景下。
...
3. 可以宣称本产品是面向 faceless YouTube creators 的 AI narration workflow。
4. 可以宣称免费层提供大量 AI starter voices，但不能宣称 unlimited free。
5. 首页和营销页应先强调具体人群、低摩擦试用和具体结果，再解释模型或音色技术细节。
6. 旗舰音色应被包装成频道人格资产与长期一致性能力，而不是单纯“海量音色数量”。
...
7. 免费用户在第一次体验时应感受到“选择很多”，但付费用户应感受到“更稳、更强、更完整”。
8. 真正的护城河是 `workflow + flagship voices + consistency`，而不是单纯堆更多音色数量。
...
默认人设音色库在 V1 中应先围绕英语完整产品和主赛道簇组织，首发重点是：

1. 英语大启动库音色
2. 英语旗舰音色
3. 围绕 `true crime`、`dark mystery`、`dark history` 的人格化入口
...
4. 文本型文档上传
5. 异步长文本生成流程
6. `story-first documentary` 主赛道内的音色包装
7. 大型 AI 启动库音色选择
8. `3-5` 个旗舰音色
9. 项目持久化
10. 音频文件下载
...
| 一级内容母类 | 中文说明 | 选择原因 |
|---|---|---|
| `Story-first Documentary` | 以故事和纪录片式叙事承载长视频 | 能同时覆盖悬疑、黑暗历史、案件叙事和相邻 story-led niche，最贴近首发用户心智 |

V1 之所以先收在这一类，是因为：

1. 它在 Reddit / X 的 faceless YouTube 讨论中反复出现
2. 对情绪、节奏和 narrator persona 的要求高，适合做音色差异化
3. 它天然依赖长文本旁白和后期配合
...
|---|---|---|
| `Story narration` | 旁白讲故事 | 真实案件、神秘故事、未解悬案 |
| `Documentary explainers` | 纪录片式解释 | 黑暗历史、历史轶事、文化谜案 |
| `Case file breakdowns` | 案件卷宗式拆解 | true crime、案件复盘、人物事件档案 |
...
| 三级热点标签 | 作用 | 典型挂载位置 |
|---|---|---|
| `true crime` | 强流量叙事标签 | 案件类故事、案例页、工具页 |
| `dark mystery` | 神秘 / 未解叙事标签 | 悬疑类音色、人设页 |
| `dark history` | 黑暗历史 / 冷门历史标签 | 历史类故事、纪录片页 |
| `unsolved` | 未解叙事标签 | 案件 / 谜案入口 |
| `case files` | 卷宗感、档案感标签 | 案件拆解和旁白人格包装 |
...
|---|---|---|---|
| `Story-first Documentary` | `Story narration` | `true crime` / `dark mystery` / `unsolved` | `true crime narration` |
| `Story-first Documentary` | `Documentary explainers` | `dark history` | `dark history documentary` |
| `Story-first Documentary` | `Case file breakdowns` | `case files` / `true crime` | `case file breakdown` |
...
1. 主推语言：英语
2. 首批只做 `3-5` 个旗舰音色
3. 旗舰音色用于体现频道人格、一致性和付费层价值

首批入口方向优先为：

1. `true crime narration`
2. `dark mystery narration`
3. `dark history documentary`

这样做的理由是：

1. 最贴 faceless YouTube 长视频创作的首发目标场景
2. 最容易形成强烈的人设差异
3. 质量评测和版本锁定成本可控
4. 能与免费大启动库形成清晰分层
...
4. 更贴合当前“英语完整产品 + 多语 SEO / 分发面 + 长任务工作流”的产品形态
5. 相比高价商业模板，当前性价比更高
...
1. 支持一次免登录真实试用
2. 免费层主要升级门槛是“长项目长度”和“旗舰音色 / 完整工作流”
3. 免费层的体验目标是让用户认真体验产品价值，同时通过大启动库建立“选择很多”的第一感受
...
1. `Free` 负责真实体验、结果带走和大启动库音色选择感，不负责完整生产。
2. `Pro` 是默认主推档，负责承接旗舰音色和完整工作流。
...
1. `Free`：强调免登录试用、音频结果带走，以及大量 AI starter voices 的选择感。
2. `Pro`：强调旗舰音色 + 完整工作流，包括 `SRT` 导出与局部修复。
...
2. `Free` 可以强调“大量启动库音色”，但不能暗示 unlimited free
3. `Pro` 强调“旗舰音色 + 完整工作流”
...
如果多语 SEO / 分发页扩张速度快于英语完整产品和内容真源维护能力，容易造成“搜索入口很多，但真实产品承诺和页面内容不一致”的问题。
...
1. 主定位：面向 faceless YouTube creators 的英语叙事型 AI narration 工作流
2. 副标题：强调 no-login real trial + finished result，而不是抽象能力描述
3. CTA：
   - 免费开始
   - 立即试一个音色 / 看赛道

#### Try First / Tool Entry
...
1. 大量可试的 AI narrator voices
2. 长项目交付
3. 更适合 faceless YouTube story-first documentary workflow
...
2. 让“免费试用 + 大启动库 + 旗舰音色 + 长文本工作流”在视觉上成立
...
4. 首页和工具页都应同时传达“试用门槛低”和“旗舰质量可升级”
...
1. `distribution-and-growth-surface` 的首页主叙事与工具页矩阵落地
2. 英语完整产品 + 多语 SEO landing 的页面边界
3. `story-first documentary` 主赛道与 `3-5` 个旗舰音色的具体命名
4. 免费大启动库与付费旗舰库的前端呈现方式
