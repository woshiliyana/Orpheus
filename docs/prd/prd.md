...
3. 第一可卖结果锁定为 `long text -> stable audio`

产品当前有两个核心价值支柱：

1. 稳定长文本工作流：从长脚本到可发布音频成品
...
2. 第一阶段首页主承诺必须围绕稳定长文本旁白，而不是广义 AI 视频自动化
...
4. 用户最终先拿到的是可用长音频成品，而不是一段“声音样本”或半手动工作流。
...
2. 第二层再解释这是 stable long-form narration workflow
...
3. 可以宣称本产品是面向 faceless educational explainer creators 的 stable long-form narration workflow。
4. 可以宣称本产品优先解决长文本稳定出旁白，但不能在第一阶段暗示自动视频、自动字幕或 unlimited free 已全部完成。
...
1. 英语大启动库音色
2. 足够支撑 hosted feasibility spike 的基础英语音色
...
1. 先做英语完整产品与 hosted feasibility 对应的主叙事
2. 再做工具页和精选多语分发入口
3. 最后再扩展更多多语增长页
...
1. 当前价格锚点只作为后续层假设：`$0 / $15 / $30`
2. 若 pricing page 开放，再提供 `Monthly / Yearly` 切换
...
2. 免费层主要升级门槛是“长项目长度”和后续 workflow layer
3. 免费层的体验目标首先是让用户理解“稳定长文本出音频”的价值，而不是先强调大量音色选择
4. 免费层当前建议量级为每月约 `10-12` 分钟音频额度，但必须在真实成本验证后再正式承诺
...
2. `Pro` 是默认主推档，负责承接更长时长和更完整 workflow layer。
...
1. `Free`：若开放，强调稳定长文本体验与结果带走。
2. `Pro`：强调更长时长、更稳的产出，以及后续完整 workflow layer，包括 `SRT` 导出与局部修复。
...
1. 主定位：面向 faceless educational explainer creators 的英语长文本旁白工作流
2. 副标题：强调 stable long-form narration + publish-ready result，而不是抽象能力描述
3. CTA：
   - 了解长文本直出
   - 申请试用 / 预约演示
...
1. 稳定长文本直出
2. 更少手动拆段和盯流程
3. 更适合 faceless educational explainer workflow
...
2. 第一执行楔子锁定为 `educational explainers + long text -> stable audio`。
3. `IndexTTS2` 是认真纳入考虑的现实候选，但不是第一放行路径。
4. 生产引擎选择目前保持开放，但当前推荐优先评估 `Inworld TTS 1.5 Max`，以 `Cartesia` 为备选。
5. 当前产品优先级仍然是创作者工作流优先，而不是模型选择优先。
6. 第一技术放行标准是“稳定长文本出音频”，不是先把 `SRT`、增长面和音色分层全部补齐。
