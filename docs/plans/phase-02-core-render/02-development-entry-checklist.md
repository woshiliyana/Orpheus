...
- [ ] The workbook at `/docs/plans/phase-02-core-render/05-pricing-unit-economics-model-v0.40.xlsx` is explicitly treated as a planning companion, not canonical truth.
...
- [ ] The exact three-script corpus is frozen: `control-short`, `control-medium`, and `control-long`.
- [ ] The corpus contains real educational-explainer scripts rather than synthetic placeholder copy.
- [ ] Provider comparison fields are frozen before testing begins: success / failure, retry behavior, latency, chunk count, stitch count, seam defects, effective cost, alignment-asset coverage, provider timestamp usefulness, and subtitle-text-fidelity risk.
